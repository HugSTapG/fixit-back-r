...
export class TecnicosController {
  ...
  async findOne(@Payload() data: { idTecnico: number }) {
    try {
      ...
      this.logger.error(
        `Error getting technician: ${data.idTecnico}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async findByUserId(@Payload() data: { idUser: number }) {
    try {
      ...
      this.logger.error(
        `Error getting technician by user ID: ${data.idUser}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async create(
    @Payload() data: { createTecnicoDto: CreateTecnicoDto; currentUser: any },
  ) {
    try {
      this.logger.log(
        `Creating technician for user: ${data.createTecnicoDto.idUser}`,
      );
      const tecnico = await this.tecnicosService.create(data.createTecnicoDto);
      return { success: true, data: tecnico };
    } catch (error) {
      this.logger.error(
        `Error creating technician: ${data.createTecnicoDto.idUser}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async update(
    ...
    try {
      ...
      this.logger.error(
        `Error updating technician: ${data.idTecnico}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async deactivate(@Payload() data: { idTecnico: number; currentUser: any }) {
    try {
      ...
      const tecnico = await this.tecnicosService.deactivate(
        idTecnico,
        currentUser,
      );
      return { success: true, data: tecnico };
      ...
      this.logger.error(
        `Error deactivating technician: ${data.idTecnico}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async findByParroquia(
    @Payload() data: { codigoParroquia: string; filterDto?: TecnicoFilterDto },
  ) {
    try {
      const { codigoParroquia, filterDto } = data;
      this.logger.log(`Getting technicians by parroquia: ${codigoParroquia}`);
      const result = await this.tecnicosService.findByParroquia(
        codigoParroquia,
        filterDto,
      );
      return { success: true, data: result };
      ...
      this.logger.error(
        `Error getting technicians by parroquia: ${data.codigoParroquia}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async findByTipoServicio(
    @Payload() data: { idTipoServicio: number; filterDto?: TecnicoFilterDto },
  ) {
    try {
      const { idTipoServicio, filterDto } = data;
      this.logger.log(`Getting technicians by service type: ${idTipoServicio}`);
      const result = await this.tecnicosService.findByTipoServicio(
        idTipoServicio,
        filterDto,
      );
      return { success: true, data: result };
      ...
      this.logger.error(
        `Error getting technicians by service type: ${data.idTipoServicio}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async findAvailableForRequest(
    ...
    try {
      ...
      this.logger.log(
        `Finding available technicians for request: ${idSolicitud}`,
      );
      ...
      this.logger.error(
        `Error finding available technicians for request: ${data.idSolicitud}`,
        error.stack,
      );
      return {
        success: false,
